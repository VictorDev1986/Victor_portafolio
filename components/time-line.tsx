                ...
                <div className="-my-6">
                    {dataAboutPage.map((data) => (
                        <div key={data.id} className="relative py-6 pl-8 sm:pl-32 group">
                            <h3 className="mb-3 text-lg sm:text-xl md:text-lg lg:text-xl font-bold sm:mb-2 text-slate-300 leading-relaxed">{data.title}</h3>
                            <div className="flex flex-col sm:flex-row items-start mb-3 
                                ...
                                <div className="text-lg sm:text-lg md:text-base lg:text-lg font-medium text-gray-300 leading-relaxed">{data.subtitle}</div>
                            </div>
                            <p className="text-slate-300 mx-auto mt-0 text-lg sm:text-lg md:text-base lg:text-lg md:mx-0 md:mb-6 leading-relaxed font-medium">{data.description}</p>
                            {data.certificateLink && (
                                <a 
                                    href={data.certificateLink} 
                                    target="_blank" 
                                    rel="noopener noreferrer"
                                    className="inline-block mt-3 text-lg sm:text-lg md:text-base lg:text-lg text-green-400 hover:text-green-300 font-medium underline transition-colors duration-200"
                                >
                                    Ver Certificado
                                </a>
                            )}

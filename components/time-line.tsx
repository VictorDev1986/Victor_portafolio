                    ...
                    {dataAboutPage.map((data) => (
                        <div key={data.id} className="relative py-6 pl-8 sm:pl-32 group">
                            ...
                            <div className="flex flex-col sm:flex-row items-start mb-1 
                                ...
                                <time className="sm:absolute left-0 translate-y-0.5 inline-flex items-center justify-center text-xs font-semibold uppercase w-20 h-6 mb-3 sm:mb-0 text-emerald-600 bg-black/50 rounded-full">{data.date}</time>
                                <div className="text-max-w-xl font-bold text-gray-400">{data.subtitle}</div>
                            </div>
                            <div className="text-slate-400">{data.description}</div>
                            {data.certificateLink && <a href={data.certificateLink} target="_blank" rel="noopener noreferrer">Ver Certificado</a>}
